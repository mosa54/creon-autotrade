...
class CreonManager(QObject):
    ...
    class CpEventClass:
        ...
        def OnReceived(self):
            ...
            try:
                ...
                data = {
                    "code": code,
                    "current_price": self.obj.GetHeaderValue(13),  # 현재가
                    "volume":        self.obj.GetHeaderValue(9),   # 누적거래량
                    "trade_value":   self.obj.GetHeaderValue(10),  # 누적거래대금
                    "high_price":    self.obj.GetHeaderValue(14),   # 고가
                    "low_price":     self.obj.GetHeaderValue(15),   # 저가
                }
                
                # 1. TradingWorker에게 데이터 전달
                if code in self.manager.realtime_subscribers:
                    subscriber_info = self.manager.realtime_subscribers[code]
                    q = subscriber_info['queue']
                    q.put(data)
                    logging.info(f"[{code}] 데이터 큐에 삽입됨 (가격: {data.get('current_price')}) – 큐 크기: {q.qsize()}")

                # 2. UI 업데이트를 위한 시그널 발생
                self.manager.ui_update_signal.emit(data)

            except Exception as e:
                logging.error(f"실시간 데이터 수신 오류: {e}")

    # <<< [추가] 실시간 시세 구독 메소드
    def subscribe_realtime(self, code: str, data_queue: queue.Queue):
        if not self.is_initialized:
            return False
        if code in self.realtime_subscribers:
            self.unsubscribe_realtime(code) # 혹시 모를 중복 구독 방지

        try:
            # 실시간 시세용 COM 객체 생성
            obj = win32com.client.Dispatch("DsCbo1.StockCur")
            # 이벤트 핸들러 연결
            handler = win32com.client.WithEvents(obj, self.CpEventClass); handler.obj = obj
            handler.manager = self # 핸들러가 CreonManager에 접근할 수 있도록 설정
            
            obj.SetInputValue(0, code)
            obj.Subscribe()

            self.realtime_subscribers[code] = {"obj": obj, "handler": handler, "queue": data_queue}
    ...
    def unsubscribe_realtime(self, code: str):
        if code in self.realtime_subscribers:
            subscriber_info = self.realtime_subscribers[code]
            subscriber_info['obj'].Unsubscribe()
    ...
    def run(self):
        ...
        try:
            while not self._stop_event.is_set():
                ...
                # 매 30분마다 N일 고점/저점 갱신
                if datetime.now().minute in [0, 30] and datetime.now().second < 5:
                    self.refresh_nday_high_targets()
                    self.refresh_nday_low_targets()
